...
**Bump `CACHE` in `sw.js` before every push** (`dadjokes-v10`, and so on).
...
```sh
# edit sw.js: const CACHE = 'dadjokes-v10';
...
strokes are measured by distance to a line segment, which gives round
marker-pen caps for free:
