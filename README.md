...
The stacked sticky notes with the `dadjokes.wtf` wordmark, matching the
promo/donation image. Source is `promo/dadjokes-icon.html` — the donation card
adapted for icon use: taglines dropped and the wordmark enlarged, because at
home-screen sizes (~60px) small text is an unreadable smudge and an app icon
shouldn't carry a donation line.

Rendered with headless Chrome (so it uses the app's real Marker Felt font),
then supersampled down to each size:

```sh
CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
"$CHROME" --headless --disable-gpu --force-device-scale-factor=2 \
  --window-size=1024,1024 --screenshot=/tmp/icon_2x.png \
  "file://$PWD/promo/dadjokes-icon.html"
for pair in 512:icon-512 192:icon-192 180:apple-touch-icon; do
  cp /tmp/icon_2x.png "icons/${pair##*:}.png"
  sips -z "${pair%%:*}" "${pair%%:*}" "icons/${pair##*:}.png"
done
```

`tools/mkicons.py` generated the earlier pure-CSS marks (a lone sticky note,
then a WTF knockout) and is kept for reference, but it is no longer the icon
source.

The matching donation card is `promo/dadjokes-donate.html` → `dadjokes-donate.png`.
