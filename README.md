...
**Bump `CACHE` in `sw.js` before every push** (`dadjokes-v12`, and so on).
...
```sh
# edit sw.js: const CACHE = 'dadjokes-v12';
...
first and slightly shorter. Only the knocked-out letters use the distance-to-
segment test, which gives them round pen caps:
