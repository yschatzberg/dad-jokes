...
**Bump `CACHE` in `sw.js` before every push** (`dadjokes-v11`, and so on).
...
```sh
# edit sw.js: const CACHE = 'dadjokes-v11';
...
The heavy stroke is a rectangle with square ends; the lighter one is a capped
segment, measured by distance to a line, which gives round pen caps for free.
Round ends on something as thick as the top stroke read as a pill rather than
a pen mark, which is why it isn't drawn the same way:
