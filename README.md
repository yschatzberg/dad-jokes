...
**Bump `CACHE` in `sw.js` before every push** (`dadjokes-v13`, and so on).
...
```sh
# edit sw.js: const CACHE = 'dadjokes-v13';
...
Both strokes are rectangles with square ends - round caps on marks this heavy
read as pills rather than pen strokes. The second is half the thickness of the
first and 86% its length, sharing a left edge with it so the two sit on a
common margin like lines of writing. Only the knocked-out letters use the
distance-to-segment test, which gives them round pen caps:
